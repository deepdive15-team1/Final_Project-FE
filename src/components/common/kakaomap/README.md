...
`react-kakao-maps-sdk`를 기반으로 성능 최적화와 사용자 편의성이 적용된 공통 지도 컴포넌트입니다.
**불필요한 리렌더링을 방지**하도록 설계되었으며, 초기 위치 자동 설정, 부드러운 이동 애니메이션(`panTo`), 경로 그리기, 그리고 다양한 지도 이벤트 핸들링을 지원합니다.

---

## ✨ 주요 기능

- **🚀 성능 최적화**: `Lazy Initialization`과 `Memoization`(`useCallback`, `useMemo`)을 적용하여 지도 깜빡임과 재렌더링을 최소화했습니다.
- **📍 부드러운 이동**: 부모 컴포넌트에서 `center` props가 변경되면, 지도가 끊김 없이 부드럽게 해당 위치로 이동합니다 (`panTo`).
- **자동 초기 위치**: 컴포넌트 마운트 시 사용자 위치(GPS)를 받아 지도의 시작점을 자동으로 설정합니다. (`center` prop이 없을 경우)
- **마커 & 드래그**: 클릭, 커스텀 이미지, **Z-Index 제어**, **드래그 후 좌표 반환** 기능을 지원합니다.
- **🗺️ 지도 이벤트 감지**: 지도의 생성(`onCreate`), 드래그 종료(`onDragEnd`), 줌 레벨 변경(`onZoomChanged`) 이벤트를 부모 컴포넌트로 전달합니다.
...
| **Line Style** | | | **경로(Polyline) 스타일 설정** |
| `lineColor` | `string` | `"#FF3300"` | 경로 선의 색상 (Hex Code 권장) |
| `lineWeight` | `number` | `5` | 경로 선의 두께 |
| `lineOpacity` | `number` | `0.8` | 경로 선의 투명도 (0~1) |
| **Map Events** | | | **지도 객체 이벤트 핸들러** |
| `onCreate` | `(map: kakao.maps.Map) => void` | `undefined` | 지도가 처음 생성되었을 때 실행됩니다. `map` 인스턴스를 반환합니다. |
| `onDragEnd` | `(map: kakao.maps.Map) => void` | `undefined` | 지도의 드래그가 끝났을 때 실행됩니다. |
| `onZoomChanged` | `(map: kakao.maps.Map) => void` | `undefined` | 지도의 줌 레벨이 변경되었을 때 실행됩니다. |
...
export interface MarkerType {
  ...
  lat: number;
  lng: number;
  content?: string; // 마커 위 말풍선 텍스트 (없으면 마커만 표시)

  // 커스텀 이미지 설정 (없으면 기본 마커)
  image?: {
    src: string;
  ...
  zIndex?: number; // 마커의 우선순위 (높을수록 위에 표시됨)
}
```
...
```
### 5. 지도 이벤트 감지 (줌/드래그/생성)
지도의 상태 변화가 필요할 때 사용합니다. (예: 지도 이동 시 현재 영역 내 데이터 다시 로드)
...
const MapEventsExample = () => {
  const handleDragEnd = (map: kakao.maps.Map) => {
    ...
    console.log("이동 후 중심:", center.getLat(), center.getLng());
    console.log("현재 영역:", bounds.toString());
    // 여기서 API 재호출 등의 로직 수행
  };

  return (
    <KaKaoMap
      onDragEnd={handleDragEnd}
      onZoomChanged={(map) => console.log("줌 레벨 변경:", map.getLevel())}
      onCreate={(map) => console.log("지도 로드 완료!")}
    />
  );
};
```